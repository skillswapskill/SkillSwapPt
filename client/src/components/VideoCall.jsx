...
// ✅ FIXED: Import useApi hook instead of direct apiClient import
...
const VideoCall = ({ sessionId, userId, userName, sessionData }) => {
  ...
  const [creditsReleased, setCreditsReleased] = useState(false)

  // ✅ FIXED: Use the useApi hook
  const { apiClient, post } = useApi()
  ...
  const handleMeetingStart = async () => {
    try {
      console.log("🚀 Starting meeting tracking...")
      
      // ✅ FIXED: Use the post method from useApi hook
      const response = await post('/api/sessions/start-meeting', {
        sessionId
      })

      if (response.data.success) {
        setMeetingStarted(true)
        toast.success("Meeting started - tracking enabled!", {
          position: "top-right",
          autoClose: 3000
        })
      ...
      toast.warning("Meeting started but there was a tracking issue")
    }
  }

  // ✅ MAIN FIX: Handle meeting end with AUTOMATIC credit release
  const handleMeetingEnd = async () => {
    if (creditsReleased) {
      console.log("Credits already released, skipping...")
      return
    }

    try {
      console.log("🎯 Ending meeting and auto-releasing credits...")
      setCreditsReleased(true)
      
      // ✅ FIXED: Use the post method from useApi hook
      const response = await post('/api/sessions/end-meeting', {
        sessionId
      })

      if (response.data.success) {
        const creditsAmount = response.data.creditsReleased || 0
        ...
        toast.success(
          <div>
            ...
            <div style={{fontSize: '13px'}}>
              {creditsAmount > 0 
                ? `${creditsAmount} credits automatically released to teacher` 
        ...
        console.log(`✅ Meeting ended and ${creditsAmount} credits released successfully`)
      }
    } catch (error) {
      console.error("Error ending meeting:", error)
      toast.error("Meeting ended but there was an issue with credit processing", {
        position: "top-center",
        autoClose: 5000
      })
    }
  ...
  const triggerContentWarning = async (analysisResult) => {
    ...
    const avgConfidence = positiveDetections.length > 0 
      ? positiveDetections.reduce((sum, d) => sum + d.confidence, 0) / positiveDetections.length 
      : 0

    // Require multiple positive detections and prevent spam
    if (positiveDetections.length >= 2 && avgConfidence > 0.7 && now - lastDetectionTime.current > 10000) {
      lastDetectionTime.current = now

      const severity = avgConfidence > 0.85 ? 'high' : 'medium'
      const warningMessage = severity === 'high' ? "Explicit Content Detected" : "Inappropriate Content Warning"
      ...
      console.log("⚠️ CONTENT WARNING:", reason)
      ...
      if (socketRef.current?.connected) {
        socketRef.current.emit('suspicious-activity-detected', {
          ...
          message: reason,
          analysisResult,
          detectionMethod: "CustomProductionAlgorithm"
        })
      }
    }
  }
    ...
    const toastContent = (
      <div>
        <div style={{fontWeight: 'bold', marginBottom: '8px'}}>
          ⚠️ WARNING #{data.warningCount} • {data.scenario} • Content Violation
        </div>
    ...
    try {
      ...
      zp.joinRoom({
        ...
        onJoinRoom: () => {
          ...
          handleMeetingStart() // ✅ Start meeting tracking
          
          toast.success("Session started - content monitoring active!", {
            position: "top-right",
            autoClose: 3000
          })
        },
        ...
        onLeaveRoom: () => {
          console.log("Left the session - ending meeting and auto-releasing credits")
          ...
          handleMeetingEnd() // ✅ End meeting and auto-release credits
      ...
      {detectionEnabled && meetingActive && connectionStatus === 'connected' && (
        <div className="ai-monitoring-production">
          <div className="monitoring-indicator-production">
            ...
            <span className="status-text-production">Content Monitoring</span>
            {warningCount > 0 && (
              <span className="warning-badge-production">{warningCount}</span>
            )}
          </div>
        </div>
      )}
